...
export default function BookingList() {
  ...
  const onCancel = async (bookingId, bookingStatus, checkinDate) => {
    try {
      ...
      // ✅ 여기(confirm 통과 직후)가 로그 넣을 위치
      console.log("취소 클릭 bookingId:", bookingId, "userId:", userId);

      await cancelBooking({ bookingId, userId });

      console.log("취소 API 호출 완료");

      alert("취소되었습니다.");
      window.location.reload();
    } catch (e) {
      console.error("[취소 실패 상세]", e);
      alert(e?.response?.data?.message ?? e?.message ?? "예약 취소에 실패했습니다.");
    }
  };
