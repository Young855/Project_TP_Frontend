...
import { axiosConfig } from "../config"; // 프로젝트에 이미 있는 공통 설정 객체 사용
// axiosConfig 예시: { baseURL: "/api", withCredentials: true, headers: { "Content-Type": "application/json" } }
const api = axios.create(axiosConfig);

/* =========================
   인증/인가 유틸 (프로젝트 규칙에 맞게 수정 가능)
   - 토큰/역할 저장 위치(localStorage/sessionStorage/쿠키)와 키 이름만 맞추세요.
   ========================= */
function getAuth() {
  try {
    const raw = localStorage.getItem("auth"); // 예: { token: "...", role: "ADMIN"|"USER" }
    if (!raw) return null;
...
export const amenityRoutes = [
  {
    ...
    children: [
      {
        index: true,
        loader: amenitiesListLoader,
        element: <AmenitiesListPage />,
      },
      {
        path: "new",
        element: <AmenityCreatePage />,
        action: amenityCreateAction,
        errorElement: <RouteErrorFallback />,
      },
      {
        path: ":amenityId",
        loader: amenityDetailLoader,
        element: <AmenityDetailPage />,
        errorElement: <RouteErrorFallback />,
      },
      {
        path: ":amenityId/edit",
        element: <AmenityEditPage />,
        action: amenityUpdateAction,
        errorElement: <RouteErrorFallback />,
      },
      {
        path: ":amenityId/delete",
        action: amenityDeleteAction,
        errorElement: <RouteErrorFallback />,
      },
    ],
  },
